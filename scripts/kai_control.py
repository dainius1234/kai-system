...
MEDIC_STATUS_PATH = Path(os.getenv("KAI_MEDIC_STATUS_PATH", "/tmp/kai_medic_status.json"))
...
def read_system_health() -> Dict[str, object]:
    if MEDIC_STATUS_PATH.exists():
        try:
            return json.loads(MEDIC_STATUS_PATH.read_text(encoding="utf-8"))
        except Exception:
            return {}
    return {}


def apply_health_fixes() -> List[str]:
    fixes: List[str] = []
    key_path = Path(os.getenv("KAI_VAULT_KEY_PATH", "/data/vault.key"))
    if key_path.exists():
        os.chmod(key_path, 0o600)
        fixes.append(f"chmod 600 {key_path}")
    return fixes
...
class KaiControlUI:
    def __init__(self) -> None:
        ...
        self.btn_health = tk.Button(self.root, text="System Health", command=self._health)
        self.btn_fix_all = tk.Button(self.root, text="Fix all", command=self._fix_all)

        for b in [self.btn_seal, self.btn_backup, self.btn_paper, self.btn_restore, self.btn_kill, self.btn_roll, self.btn_logs, self.btn_advisor, self.btn_health, self.btn_fix_all]:
            b.pack_forget()
    ...
    def _health(self) -> None:
        ...
        quarantined = status.get("suspicious_quarantined") or []
        lines = [
            "System Health",
            f"Unsafe: {status.get('unsafe')}",
            f"Lynis score: {status.get('lynis_score')}",
            f"Infected files: {status.get('infected_files')}",
            "Quarantine:",
        ]
        lines.extend([f"- {x}" for x in quarantined] or ["- empty"])
        messagebox.showinfo("System Health", "\n".join(lines)[:3800])

    def _fix_all(self) -> None:
        if not messagebox.askyesno("kai-control", "Apply suggested hardening fixes?"):
            return
        fixes = apply_health_fixes()
        messagebox.showinfo("kai-control", "Applied fixes:\n" + ("\n".join(fixes) if fixes else "No actionable fixes found"))
