...
async def run_graph(request: GraphRequest) -> GraphResponse:
    ...
    saver.save_episode({"user_id": "keeper", "ts": time.time(), "input": request.user_input, "output": plan.get("summary", ""), "outcome_score": 1.0 if gate_decision else 0.7})
...
async def recall_last_episode(req: EpisodeRequest) -> Dict[str, Any]:
    ...
    raw_context = "\n".join(f"[{e.get('ts')}] IN={e.get('input')} OUT={e.get('output')}" for e in episodes)
