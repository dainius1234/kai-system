...
MEDIC_STATUS_PATH = os.getenv("KAI_MEDIC_STATUS_PATH", "/tmp/kai_medic_status.json")


def load_medic_status() -> Dict[str, Any]:
    try:
        with open(MEDIC_STATUS_PATH, "r", encoding="utf-8") as f:
            data = json.load(f)
...
async def run_graph(request: GraphRequest) -> GraphResponse:
    ...
    medic = load_medic_status()
    if medic.get("unsafe"):
        conviction = min(conviction, float(medic.get("conviction_limit", 8.4)))
        plan["safety_block"] = "Unsafe state detected by medic scan — fix system health before execution."
