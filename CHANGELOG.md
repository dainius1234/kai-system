...
- **Cognitive Reasoning Pipeline FSM** (`agentic/cognitive_fsm.py`): deterministic spine for multi-agent reasoning. States: GATHER → DEBATE → FACT_CHECK → CAUSAL_CHECK → CONVICTION_GATE → PRESENT. Safety invariants: every loop has `max_retries` cap, every state has timeout, exhausted retries → HALT (escalate to operator). `AgentHandoff` schema-validated handoff with `HandoffStatus` enum (COMPLETE/PARTIAL/FAILED/CONSENSUS/NO_CONSENSUS/PASS/FAIL). Per-swarm configs (`SwarmConfig`): `trading` (high threshold, tight timeouts), `research` (loose caps, depth over speed), `skill_forge` (iterative), `default`. `CognitiveFSM.run()` executes the full pipeline with transition log.
- **Persistent Teammates** (`agentic/teammates.py` + `data/teammates/`): four named cognitive personas each in a `.md` file with specialty, description, system prompt. **Scout** (skill discovery), **Doctor** (system health diagnosis), **Sage** (reflection + counterargument, complements debate engine), **Oracle** (prediction + trend extrapolation). `load_teammates()`, `get_teammate(slug)`, `list_teammates()`, `build_teammate_context(slug)`. Loaded at startup (gated by `FF_PERSISTENT_TEAMMATES`). New endpoint `POST /chat/teammate/{name}` + `GET /teammates`.
- **House Doctor service** (`house-doctor/`, port 8046, `172.20.0.35`): 9-rule differential diagnosis table (D001–D009). `_classify_observations()` tags cpu_high, ram_high, docker_unhealthy, aq_degraded, sensor_anomaly, calendar_soon. `_apply_rules()` returns severity-ordered diagnoses. `POST /diagnose` writes `medical_report` to memu-core + calls notify-service for WARNING/CRITICAL. `GET /rules` exposes rule set. Integrated into proactive observer. Gated by `FF_HOUSE_DOCTOR`.
- **Capability Gap Logging** (D89/C1): `_gap_log: Counter` in agentic. Reactive hunt fires only after `GAP_HUNT_THRESHOLD` (default 3) misses for the same gap. Prevents wasted hunts on one-off unusual requests.
- **Skill Provenance + Probationary Period** (D89/C2, `skill-hunter/app.py`): every generated skill `.md` file gets YAML front-matter with `pypi_verified: true`, `hunted_at` ISO timestamp, `probationary: true`. Sidecar `.meta.json` tracks `error_count` + `disabled`. `POST /skill/{name}/error` increments error count; disables at `DISABLE_THRESHOLD` (default 3). `GET /skill/{name}/health`. Skill loader skips disabled skills.
- **World Model Provenance Layer** (D89/C3): world_state entries now `{value, source, timestamp, confidence}` per field. AQ confidence 0.9 (live) vs 0.3 (stale). Includes `emotional_context: {indicators: [], predicted_mood: null, confidence: 0.0}` as D89/D foundation. `fsm_state` also written per cycle.
- **Emergent Ritual Discovery** (D89/C5): `_propose_ritual()` fires via `asyncio.create_task` when pattern count reaches ≥7/10 cycles. Appends structured entry to `/data/RITUALS.md`. Each pattern proposed only once per process lifetime via `_proposed_rituals` set. Gated by `FF_RITUAL_DISCOVERY`.
- **Counterfactual Rehearsal Foundation** (D89/A, `agentic/counterfactual.py`): `rehearse(decision, world_state)` → `{scenarios: [], status: "stub_pending_gpu"}`. `can_rehearse()` → False. `rehearse_skill_test()` stub. Interface locked; Phase 1 LLM slots in without breaking changes.
- **Trust Negotiation Foundation** (D89/B, `tool-gate/app.py`): `POST /gate/autonomy/request` with `AutonomyRequest` model (`task, requested_level 1–5, rationale, time_limit_seconds`). Returns `pending_approval` — all requests require explicit operator grant. Logs to ledger. Gated by `FF_TRUST_NEGOTIATION`.
- **Predictive Empathy Foundation** (D89/D): `emotional_context` key in world model provenance — `{indicators, predicted_mood, confidence, note: "stub_pending_emotional_memory"}`. Activates when emotional memory accumulates in Phase 1.
- **Resource-Aware Curiosity Foundation** (D89/F, `agentic/curiosity.py`): `idle_curiosity_tick(world_state, is_gpu_available=False)` → None (stub). `CURIOSITY.md` created at `/data/CURIOSITY.md`. Tick called from proactive observer on IDLE state. No-ops in Phase 0. Gated by `FF_CURIOSITY`.
- **8 new feature flags** in `common/feature_flags.py`: `FSM`, `PERSISTENT_TEAMMATES`, `HOUSE_DOCTOR`, `RITUAL_DISCOVERY`, `GAP_LOGGING`, `TRUST_NEGOTIATION`, `PREDICTIVE_EMPATHY`, `CURIOSITY` — all default True.
- **D89 logged in `kai-pm/DECISIONS.md`**: full design rationale with rationale section on "why lay foundations early."
- **47-test suite** (`scripts/test_d89_cognitive_depth.py`): System FSM (9), Cognitive FSM (6), Teammates (5), Counterfactual stub (4), Curiosity stub (2), House Doctor (7), Skill Hunter provenance (5), D89 feature flags (9). Makefile target `test-d89-cognitive-depth` + CI step. 47/47 passing.
