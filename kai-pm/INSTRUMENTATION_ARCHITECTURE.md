...
| `KAI-GATE-021` | MEDIUM | **156 exception handlers in production discard the reason** (`except Exception: pass`), plus 8 in tests. Measured by AST across the repository. Not hypothetical: `test_soul_identity` carried one, and the moment it was made to *record* the exception instead, the message named the cause in a single line — `No module named 'system_fsm'` — turning 4 failures into 0. The same shape in production means a service degrades silently and the operator learns nothing. Many are legitimate best-effort paths, so this needs reading, not a blanket edit | **OPEN** — measured, registered, not gated. The prevention shape is the H-5 ratchet: record 156 as a baseline that may only fall |
