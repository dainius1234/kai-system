...
| TurboVec / TurboQuant | Assess | 2026-06-19 | Verified real (11.9k stars, MIT, ICLR-2026-backed), 16x compression at 2-bit. Standalone in-process index with its own file format — no documented pgvector integration. Architectural decision made (D14): Postgres holds metadata only, TurboVec's `IdMapIndex` owns similarity search. **Implemented 2026-06-19 (D14):** `TurboVecStore` class added to `memu-core/app.py` behind `VECTOR_STORE=turbovec`; `pip install turbovec` confirmed sufficient (no Rust toolchain needed). Stays at Assess, not Adopt, until live-Postgres-verified (`scripts/test_memu_turbovec.py` exists but skips without `PG_URI`/`turbovec` installed) | 2026-08-01 |
