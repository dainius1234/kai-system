...
17. Added Ed25519 weekly auto-rotation tooling (`scripts/auto_rotate_ed25519.py` + `scripts/weekly_ed25519_rotation.sh`) with dual-sign overlap mode and old-key drop on subsequent cycle.
18. Added offline market price cache plumbing (`common/market_cache.py`, `scripts/market_price_cache.py`) and advisor hinting for rising fuel trend.
19. Added heartbeat watchdog for CPU/GPU temperature and CPU load pressure with auto-sleep + Telegram alert path.
20. Added OCR receipt parser script (`scripts/ocr_receipt.py`) and advisor voice-style prompt cue in Kai control.
21. Added Merkle root endpoint for ledger tamper visibility (`GET /ledger/merkle`).
